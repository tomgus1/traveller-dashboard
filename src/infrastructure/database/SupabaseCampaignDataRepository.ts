...
import type { CampaignDataRepository } from '../../core/repositories/CampaignDataRepository';
...
export class SupabaseCampaignDataRepository implements CampaignDataRepository {
    ...
    async getCampaignFinances(campaign_id: string): Promise<FinanceRow[]> {
    ...
    async addCampaignFinance(campaign_id: string, finance: Omit<FinanceRow, 'Running Total'>): Promise<FinanceRow> {
        const { data, error } = await this.supabase
            .from('campaign_finances')
            .insert({
                campaign_id,
    ...
    async getShipFinances(campaign_id: string): Promise<FinanceRow[]> {
        const { data, error } = await this.supabase
            .from('ship_finances')
            .select('*')
            .eq('campaign_id', campaign_id)
    ...
    async addShipFinance(campaign_id: string, finance: Omit<FinanceRow, 'Running Total'>): Promise<FinanceRow> {
        const { data, error } = await this.supabase
            .from('ship_finances')
            .insert({
                campaign_id,
    ...
    async getShipCargo(campaign_id: string): Promise<CargoRow[]> {
        const { data, error } = await this.supabase
            .from('ship_cargo')
            .select('*')
            .eq('campaign_id', campaign_id)
    ...
    async addShipCargo(campaign_id: string, cargo: CargoRow): Promise<CargoRow> {
        const { data, error } = await this.supabase
            .from('ship_cargo')
            .insert({
                campaign_id,
    ...
    async getShipMaintenance(campaign_id: string): Promise<MaintenanceLogRow[]> {
        const { data, error } = await this.supabase
    ...
    async addShipMaintenance(campaign_id: string, maintenance: MaintenanceLogRow): Promise<MaintenanceLogRow> {
        const { data, error } = await this.supabase
            .from('ship_maintenance')
            .insert({
                campaign_id,
    ...
    async getCampaignLoans(campaign_id: string): Promise<LoanRow[]> {
        const { data, error } = await this.supabase
            .from('campaign_loans')
            .select('*')
            .eq('campaign_id', campaign_id)
    ...
    async addCampaignLoan(campaign_id: string, loan: LoanRow): Promise<LoanRow> {
        const { data, error } = await this.supabase
            .from('campaign_loans')
            .insert({
                campaign_id,
    ...
    async getPartyInventory(campaign_id: string): Promise<InventoryRow[]> {
        const { data, error } = await this.supabase
    ...
    async addPartyInventoryItem(campaign_id: string, item: InventoryRow): Promise<InventoryRow> {
        const { data, error } = await this.supabase
            .from('party_inventory')
            .insert({
                campaign_id,
    ...
    async getCampaignAmmo(campaign_id: string): Promise<AmmoRow[]> {
        const { data, error } = await this.supabase
            .from('campaign_ammo')
            .select('*')
            .eq('campaign_id', campaign_id)
    ...
    async addCampaignAmmo(campaign_id: string, ammo: AmmoRow): Promise<AmmoRow> {
        const { data, error } = await this.supabase
            .from('campaign_ammo')
            .insert({
                campaign_id,
    ...
    async getAllCampaignData(campaign_id: string): Promise<{
        finances: FinanceRow[];
        ...
        const [finances, shipFinances, cargo, maintenance, loans, partyInventory, ammo] =
            await Promise.all([
                this.getCampaignFinances(campaign_id),
                this.getShipFinances(campaign_id),
                this.getShipCargo(campaign_id),
                this.getShipMaintenance(campaign_id),
                this.getCampaignLoans(campaign_id),
                this.getPartyInventory(campaign_id),
                this.getCampaignAmmo(campaign_id),
            ]);
